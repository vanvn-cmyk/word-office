...
struct LibraryViewModelTests {
    ...
    func setStatusPersists() async {
        ...
        await vm.setStatus(.done, for: "doc-1")

        #expect(store.entries.first?.metadata.status == .done)
        #expect(metadata.storage["doc-1"]?.status == .done)
    }
    ...
    func setStatusUnknownID() async {
        ...
        await vm.setStatus(.done, for: "bogus")
    ...
    @Test("recordOpen promotes Draft to Reviewed (2026-09-14 — reverses arch trap #2 by explicit user decision)")
    func recordOpenPromotesDraftToReviewed() async {
        store.entries = [makeEntry(id: "doc-1", status: .draft)]

        await vm.recordOpen("doc-1")

        #expect(store.entries.first?.metadata.status == .reviewed)
        #expect(metadata.storage["doc-1"]?.status == .reviewed)
    }

    @Test("recordOpen does NOT change status once past Draft (no downgrade, no redundant re-fire)")
    func recordOpenLeavesReviewedAndDoneAlone() async {
        store.entries = [
            makeEntry(id: "doc-1", status: .reviewed),
            makeEntry(id: "doc-2", status: .done)
        ]

        await vm.recordOpen("doc-1")
        await vm.recordOpen("doc-2")

        #expect(store.entries.first { $0.id == "doc-1" }?.metadata.status == .reviewed)
        #expect(store.entries.first { $0.id == "doc-2" }?.metadata.status == .done)
    }
