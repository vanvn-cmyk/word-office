...
struct RootView: View {
    ...
    private var libraryShell: some View {
        if let libraryVM, let permissionVM {
            ...
            .sheet(item: $editingRef) { ref in
                LibraryEditorSheet(container: container, ref: ref)
            }
        } else {
            checkingView
        }
    ...
    private func closeFABMenu() {
        ...
        withAnimation(reduceMotion ? nil : .easeOut(duration: 0.15)) { isFABMenuOpen = false }
    }
        ...
        GlassEffectContainer(spacing: DSSpacing.sm) {
            HStack(alignment: .bottom, spacing: DSSpacing.sm) {
                HStack(spacing: 2) {
                    ...
                    tabBarButton(.tools, label: "Tools", systemImage: "IconPencilCase", isCustomAsset: true)
        ...
        return Button {
            ...
            VStack(spacing: 3) {
                if isCustomAsset {
                    ...
                    // filled/outline variant to switch between, so
                    // selection reads through color + the capsule
                    // background alone, same as the label text below it.
                    Image(systemImage)
...
/// Sheet wrapper for the Library-row editor — owns its own `dismiss`
/// environment so the Done button closes the sheet via `dismiss()` instead
/// of writing to the parent's `editingRef` binding from outside (skill
/// guidance: "sheets own their actions"). Also hosts its own
/// `.toastHost(_:)` so any toast fired while the editor is up stays
/// visible — sheets sit above the scene-root overlay in z-order.
private struct LibraryEditorSheet: View {
    let container: DependencyContainer
    let ref: DocumentRef
...
#Preview("Root — checking") {
    ...
    return RootView(container: DependencyContainer())
        ...
        // LibraryEditorSheet, ToolsEditorSheet) read the presenter via
        // `@Environment` and would trap-fatalError if it were missing in
        // the preview once state advances beyond `.checking`.
