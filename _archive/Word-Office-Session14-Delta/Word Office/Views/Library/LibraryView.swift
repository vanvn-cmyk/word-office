...
struct LibraryView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(item: $exportingRef, onDismiss: { exportingRef = nil }) { ref in
                DocumentPickerExporter(urls: [ref.url])
                    .ignoresSafeArea()
            }
        }
    }
        ...
        Group {
            if viewModel.searchResults.isEmpty {
                ...
                List {
                    ...
                    // See main libraryList — same fake spacer pattern.
                    Section {
                        Color.clear
                            .frame(height: 120)
                            .listRowInsets(EdgeInsets())
    ...
    private var premiumButton: some View {
        // Opens `PaywallView` — a real screen now, not a `TODO(paywall)`
        // placeholder. The 7 iap_v1-iap_v7 variants in
        // product-strategy-master.md still aren't wired to real StoreKit
        // products (plan selection there is a local, unwired placeholder
        // per the user's own note) — only the entry point + screen exist.
        //
        // `crown.fill` — Apple's own subscription-entry convention (App
        // Store, News+, Podcasts). Gold gradient chip (not a flat glyph)
        // because a plain nav-bar icon reads as a filter/settings toggle;
        // a badge signals "this is a purchase". `dsPremiumGoldStart/End`
        // mirror Apple's systemYellow → systemOrange pair — the first
        // attempt reused `dsStatusWarning`, which is really a muted
        // brown/amber (A75D00) meant for "draft" status pills, not a
        // premium gold; it read as a flat bronze circle, not gold.
        Button {
            ...
            ZStack {
                Circle()
                    ...
                    // Top-only ring highlight — the gradient fades to
                    // `.clear` by the vertical center, so only the upper
                    // half of the ring actually shows, reading as a light
                    // source from above. Same technique as `ToolsTabView`'s
                    // `IconBadge`, for a consistent "dimensional" feel.
                    .overlay(
                        Circle().strokeBorder(
                            LinearGradient(
                    ...
                    .shadow(color: Color.dsPremiumGoldEnd.opacity(0.5), radius: 5, y: 2)
