...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        NavigationStack {
            ...
            // Form's default top inset before the first section is what
            // was pushing the banner so far below "Settings" — the same
            // gap the compact section spacing above doesn't touch, since
            // that only affects space *between* sections, not before the
            // first one.
            .contentMargins(.top, DSSpacing.xxs, for: .scrollContent)
            .autoHidesTabBarOnScroll()
            .prominentInlineTitle("Settings")
