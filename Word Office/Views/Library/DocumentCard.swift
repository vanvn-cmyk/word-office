        ...
        Button {
            ...
            HStack(spacing: DSSpacing.sm) {
                ...
                VStack(alignment: .leading, spacing: DSSpacing.xxs) {
                    ...
                    // Reviewed/Done), so repeating it on every row inside
                    // its own section was redundant. Stays defined below,
                    // unused, in case a row-level need for it comes back
                    // (e.g. mixed-status contexts like search results).
                    //
    ...
    var body: some View {
        ...
        List {
            ...
            DocumentCard(entry: LibraryEntry(
                ...
                metadata: DocumentMetadata(id: "3", status: .reviewed, lastOpenedAt: now, lastModifiedAt: now),
                downloadState: .local
            ))
        }
        .listStyle(.plain)
